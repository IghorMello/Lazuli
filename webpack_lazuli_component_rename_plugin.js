...
const { add_lazuli_prefix_to_polymer_elements, list_all_custom_polymer_elements } = require('./add_lazuli_prefix_to_polymer_elements')
...
function LazuliComponentRenamePlugin(options) {
}

LazuliComponentRenamePlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('emit', async function (compilation, callback) {
    ...
    for (let filename of Object.keys(compilation.assets)) {
      ...
      if (source_text === filename_to_cached_source[filename]) {
        ...
        output_text = 'window.lazuli_content_script = true;\n\n' + add_lazuli_prefix_to_polymer_elements(source_text, list_of_components_to_rename)
...
module.exports = LazuliComponentRenamePlugin
