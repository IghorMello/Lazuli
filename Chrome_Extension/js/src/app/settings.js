...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("restrictionsBtn")
  ...
  document
    .getElementById("file-input-backup")
    .addEventListener("change", function (e) {
      restore(e);
    });
  document
    .getElementById("addBlackSiteBtn")
    .addEventListener("click", function () {
      addNewSiteClickHandler(
        "addBlackSiteLbl",
        null,
        actionAddBlackSiteToList,
        "notifyForBlackList"
      );
    });
  document
  ...
  document
    .getElementById("addNotifySiteBtn")
    .addEventListener("click", function () {
      addNewSiteClickHandler(
        "addNotifySiteLbl",
        "addNotifyTimeLbl",
        actionAddNotifyToList,
        "notifyForNotifyList"
      );
    });
  document
    .getElementById("viewTimeInBadge")
    .addEventListener("change", function () {
      storage.saveValue(SETTINGS_VIEW_TIME_IN_BADGE, this.checked);
    });
  document
    .getElementById("blockDeferral")
    .addEventListener("change", function () {
  ...
  document
    .getElementById("rangeToDays")
    .addEventListener("change", function () {
      storage.saveValue(SETTINGS_INTERVAL_RANGE, this.value);
    });
  document
    .getElementById("grantPermissionForYT")
    .addEventListener("click", function () {
      grantPermissionForYT();
    });
  document
    .getElementById("grantPermissionForNetflix")
    .addEventListener("click", function () {
      grantPermissionForNetflix();
    });
  document
    .getElementById("grantPermissionForNotifications")
    .addEventListener("click", function () {
      grantPermissionForNotifications();
    });
  document
    .getElementById("notifyMessage")
    .addEventListener("change", function () {
...
function loadSettings() {
  ...
  storage.getValue(STORAGE_RESTRICTION_LIST, function (items) {
    restrictionList = (items || []).map(
      (item) => new Restriction(item.url || item.domain, item.time)
    );
    viewRestrictionList(restrictionList);
  });
  storage.getValue(STORAGE_NOTIFICATION_LIST, function (items) {
    notifyList = (items || []).map(
      (item) => new Notification(item.url || item.domain, item.time)
    );
...
function setUIForAnyPermissionForYT() {
  ...
  document
    .getElementById("permissionSuccessedBlockForYT")
...
function setUIForAnyPermissionForNetflix() {
  ...
  document
    .getElementById("permissionSuccessedBlockForNetflix")
    .classList.add("inline-block");
...
function setUIForAnyPermissionForNotifications() {
  document.getElementById(
    "permissionSuccessedBlockForNotifications"
  ).hidden = false;
  document
    .getElementById("permissionSuccessedBlockForNotifications")
    .classList.add("inline-block");
  ...
  if (items !== undefined) {
    for (var i = 0; i < items.length; i++) {
      addDomainToEditableListBox(
        ...
        "notifyList",
        actionEditSite,
        deleteNotificationSite,
        updateItemFromNotifyList,
...
function actionAddNotifyToList(newSite, newTime) {
  if (!isContainsNotificationSite(newSite)) {
    ...
    if (notifyList === undefined) notifyList = [];
...
function addNewSiteClickHandler(lblName, timeName, actionCheck, notifyBlock) {
  ...
  if (
    ...
    if (!actionCheck(newSite, newTime)) viewNotify(notifyBlock);
  }
}
...
function actionEditSite(e, actionUpdateTimeFromList, actionUpdateList) {
  ...
  if (timeElement.classList.contains("clock-li-readonly")) {
    ...
    editCmd.src = "/assets/img/success.png";
    $(".clockpicker").clockpicker();
  } else {
    ...
    if (domain !== "" && time !== "") {
      var editCmd = targetElement.querySelector('[name="editCmd"]');
      editCmd.src = "/assets/img/edit.png";
...
function updateItemFromRestrictionList(domain, time) {
  restrictionList.find((x) => x.url.isMatch(domain)).time =
    convertTimeToSummaryTime(time);
}

function updateItemFromNotifyList(domain, time) {
  notifyList.find((x) => x.url.isMatch(domain)).time =
    convertTimeToSummaryTime(time);
}
