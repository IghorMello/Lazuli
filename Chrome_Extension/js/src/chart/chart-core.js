  ...
  function chart(selection) {
    selection.each(function (data) {
      ...
      var svg = selection
        ...
        .attr(
          "transform",
          "translate(" + (width / 2 - 105) + "," + (height / 2 + 12) + ")"
        );
      // ===========================================================================================
      ...
      function toolTipHTML(data) {
        ...
        for (var key in data.data) {
          ...
          var className = "";
          if (key === "percentage") className = 'class="percentageValue"';
          ...
          if (i === 0) tip += '<tspan x="0">' + value + "</tspan>";
            ...
            tip +=
              '<tspan x="0" dy="1.2em"' + className + ">" + value + "</tspan>";
          i++;
        }
      ...
      function angleIsInRangeDifference(tempAngle, currentAngle, difference) {
        return (
          currentAngle < tempAngle + difference &&
          currentAngle > tempAngle - difference
        );
      }
      // ===========================================================================================
...
function barChart(data) {
  ...
  var tip = d3
    ...
    .html(function (d) {
      if (data.length > 9)
        return (
          "<strong><span class='red-label'>" +
          new Date(d.date).toLocaleDateString() +
          "</span></strong></br><strong>" +
          convertShortSummaryTimeToString(d.total) +
          "</strong>"
        );
      else
        return (
          "<strong>" + convertShortSummaryTimeToString(d.total) + "</strong>"
        );
    });
  ...
  svg
    ...
    .attr("class", "bar")
    .attr("x", function (d) {
      return x(new Date(d.date).toLocaleDateString());
    })
  ...
  if (data.length > 9)
    document.querySelectorAll("#barChart g.tick ").forEach((element) => {
      element.remove();
    });
}
...
function drawIntervalChart(data) {
  data.forEach(function (item) {
    ...
    if (hFrom != hTo) {
      ...
      data.push({
        domain: item.domain,
        interval: item.interval.split("-")[0] + "-" + timeTo,
      });
      data.push({
        domain: item.domain,
        interval: timeFrom + "-" + item.interval.split("-")[1],
      });
    }
  });
  ...
  //vertical axis
  var yAxis = d3.axisLeft().ticks(10).scale(y);

  var x = d3.scaleLinear().domain([0, 24]).range([0, width]);

  //vertical axis
  var xAxis = d3.axisBottom().ticks(24).scale(x);
  ...
  svg
    .append("g")
    .attr("class", "grid")
    .call(make_y_axis().tickSize(-width, 0, 0));
  ...
  svg
    ...
    .attr("height", function (d) {
      ...
      if (offset <= 0) {
        var offsetSeconds =
          getSecondsTo(d.interval) - getSecondsFrom(d.interval);
  ...
  function make_x_axis() {
    return d3.axisBottom().scale(x).ticks(24);
  }

  function make_y_axis() {
    return d3.axisLeft().scale(y).ticks(10);
  }
