      ...
      function (e) {
        ...
        create_intervention_dialog.addEventListener('remove_intervention_card', async function (event) {
          ...
          if (response.success) {
            ...
            swal("Falha ao remover o toque, abra um tíquete!")
          }
        })
      })
      ...
      async function (e) {
        ...
        for (var i = 0; i < 1000; i++) {
          if (localStorage['saved_intervention_' + temp_name]) {
            if (localStorage['saved_intervention_' + temp_name].trim() == self.code) {
              swal("Deslocamento já encontrado em " + temp_name + ", nada mudou.");
              return
            }
          } else {
            break;
        ...
        if (localStorage['saved_intervention_' + self.name]) {
          if (localStorage['saved_intervention_' + self.name].trim() == self.code) {
            swal("Alerta já encontrado, não há modificações.")
            return
          }

          swal("Você já tem uma intervenção com esse nome. Isso será salvo como " + self.name + "_new.");
        ...
        try {
          ...
          */
          ...
          swal('Error: Falhas do Download (' + err + ')')
        }
