...
{add_log_feedback, add_log_interventions, add_log_lazuli_disabled, add_log_lazuli_enabled} = require 'libs_backend/log_utils'
...
polymer_ext {
  ...
  help_icon_clicked: ->>
    ...
    swal {
      ...
      html: '''
      O Lazuli o ajudará a atingir seu objetivo, mostrando a você um <i> cutucão </i> diferente, como um bloqueador de feed de notícias ou um carregador de página atrasado, cada vez que você visitar o site de seu objetivo.
...
############# STRESS INTERVENTION FUNCTIONS
  ...
  survey_button_clicked: ->>
    ...
    userid = await get_user_id()
    chrome.tabs.create {url: survey_data.url + '?lazuli_userid=' + userid + '&click_location=dropdown'}
