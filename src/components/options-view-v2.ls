...
polymer_ext {
  ...
  compute_sidebar_items: (enabled_goal_info_list, goal_name_to_icon) ->
    ...
    output = [{name: msg('Visão Geral'), icon: chrome.extension.getURL('icons/results.svg')}, {name: msg('Configurações'), icon: chrome.extension.getURL('icons/configure_black.svg')}]
    for x in enabled_goal_info_list
      output.push {name: x.sitename_printable, icon: x.icon ? goal_name_to_icon[x.name] ? default_icon}
    output.push({name: msg('Ajuda / FAQ'), icon: chrome.extension.getURL('icons/help.svg')})
    return output
  
  enable_lazuli_button_clicked: ->>
  ...
  #   await swal {'title':"Welcome to Lazuli!", 'text': "Lazuli is a Chrome Extension that will help prevent you from getting distracted on the web.
  ...
  #         <b>Privacy: </b>In order to optimize your interventions, Lazuli needs to <u>modify your webpages</u> and <u>send data to our server</u> about where and when you see interventions.\n
  #         \n<b>Icons: </b>A <lazuli-logo-v2 style='zoom: 0.5;  padding-left: 5px; padding-right: 5px; display: inline-block'></lazuli-logo-v2> is placed on every intervention, so you can easily identify which elements are from Lazuli. Click the gear to get more information or disable the intervention.\n
  ...
  rerender: ->>
    self = this
    #self.is_lazuli_disabled = not (await is_lazuli_enabled())
    is_lazuli_enabled().then (is_enabled) -> self.is_lazuli_disabled = !is_enabled
    [
      goals
      enabled_goals
      goals_list
