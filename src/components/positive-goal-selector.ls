...
polymer_ext {
  ...
  properties: {
    ...
    # title_text: {
    #   type: String
    #   value: msg("We will only nudge when you are on sites you want to Gaste menos tempo em.")
    # }
    ...
    title: {
      type: String
      value: msg("Quais sites você gostaria de usar mais?")
    }
  ...
  daily_goal_help_clicked: ->
    swal {
      title: 'Como o Lazuli me ajudará a usá-los mais?'
      text: 'Com uma variedade de diferentes alertas, como colocar sua próxima aula de Duolingo na barra lateral do YouTube quando um anúncio estiver sendo executado ou colocar um lembrete amigável no feed de notícias do Facebook para ir ao site de sua escolha.'
    }
    ...
    if not checked
      ...
      check_if_first_goal = ->>       
        if !localStorage.first_goal?
          ...
          #     text: 'Lazuli will use its algorithms to try different interventions on your webpages, and intelligently figure out what works best for you. You can manually tinker with settings if you\'d like.'
          #     type: 'success'
  ...
  remove_clicked: (evt) ->>
    ...
    swal {
      ...
      html: $('<div>').append([
        $('<div>').text('A meta que você selecionou é integrada, por isso foi desativada, mas não removida:')
        $('<div>').text(goal_description)
      ])
    }
