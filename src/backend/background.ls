...
do !->>
  ...
  export get_last_visit_to_website_timestamp = ->>
    history_search_results = await new Promise -> chrome.history.search({text: 'https://lazuli.stanford.edu', startTime: 0}, it)
  ...
  export get_last_visit_to_chrome_store_timestamp = ->>
    history_search_results = await new Promise -> chrome.history.search({text: 'https://chrome.google.com/webstore/detail/lazuli/obghclocpdgcekcognpkblghkedcpdgd', startTime: 0}, it)
    last_visit_timestamp = -1
  ...
  do !->>
    ...
    if tab_info?
      if tab_info.url == 'https://lazuli.netlify.com/#installing' or tab_info.url == 'https://lazuli.stanford.edu/#installing' or tab_info.url == 'https://lazuli.github.io/#installing' or tab_info.url == 'http://lazuli.netlify.com/#installing' or tab_info.url == 'http://lazuli.stanford.edu/#installing' or tab_info.url == 'http://lazuli.github.io/#installing'
    ...
    #post_json('https://lazuli.herokuapp.com/add_secret', {user_id, user_secret})
    #setInterval ->
    #  show_finish_configuring_notification_if_needed()
    #, 5000
  ...
  {
    is_lazuli_enabled_sync
  } = require 'libs_backend/disable_lazuli_utils'
    ...
    if (not is_from_history) and (changeInfo?) and (changeInfo.status == 'loading')
      load_intervention_for_location(url, tabId).then ->
        ...
        if loaded_interventions? and loaded_interventions.length > 0
          ...
          if is_lazuli_enabled_sync() and !is_it_outside_work_hours()
            chrome.browserAction.setIcon {tabId: tabId, path: chrome.extension.getURL('icons/icon.svg')}
          else
            chrome.browserAction.setIcon {tabId: tabId, path: chrome.extension.getURL('icons/icon_disabled.svg')}
  ...
  */
  ...
  decode_lazuli_uninstall_url_data = (url) ->>
    data = url.substr(url.indexOf('/bye#') + 5) # 'https://lazuli.github.io/bye#'.length
    base64_js = require('base64-js')
    msgpack_lite = require('msgpack-lite')
  ...
  set_lazuli_uninstall_url = ->>
    lazuli_uninstall_url = await get_lazuli_uninstall_url()
    chrome.runtime.setUninstallURL(lazuli_uninstall_url)
  
  export open_lazuli_uninstall_url = ->>
  ...
  restart_failed_priority_to_counts = [
    0 # have lazuli tab open and active
    0 # have lazuli intervention running and active
    0 # have lazuli intervention open
    0 # have lazuli tab open
  ]
  ...
  record_restart_failed = (priority) !->
    ...
    if (priority == 3) and (restart_failed_priority_to_counts[0] + restart_failed_priority_to_counts[1] + restart_failed_priority_to_counts[2] + restart_failed_priority_to_counts[3]) >= 50
      restart_lazuli()
  export try_to_restart_lazuli_now = !->>
    ...
    for tab_info in active_tabs
      if tab_info.url? and tab_info.url.startsWith(chrome.extension.getURL(''))
        # have a lazuli tab open currently, active
    ...
    for tab_info in open_tabs
      if tab_info.url? and tab_info.url.startsWith(chrome.extension.getURL(''))
        # have a lazuli tab open currently, not active
        return record_restart_failed(3)
    chrome.runtime.reload()
    chrome.runtime.restart()

  lazuli_restarter_running = false
  export start_trying_to_restart_lazuli = !->>
  ...
  } = require 'libs_backend/lazuli_update_utils'
  ...
  chrome.runtime.onUpdateAvailable.addListener (update_details) ->
    if semver.valid(update_details.version)
      ...
      start_trying_to_restart_lazuli()
