    ...
    <div class="beginBox">
      <div class="contentContainer">
        ...
        <center>
          ...
          <p>
          <paper-button class="okButton" on-click="okClick"> Restrict me to {{time}} minutes!</paper-button><div class="logo"><lazuli-logo-v2 class="logo"></lazuli-logo-v2></ddiv>
        </center>
      </div>
    </div>
  </template>
...
function addBeginDialog(message) {
  ...
  const $logo = $('<center><lazuli-logo-v2></lazuli-logo-v2></center>').css({
    'margin-bottom': '80px'
  });
...
function addEndDialog(message) {
  ...
  $cheatButton.text("Sim, vou fazer isso!")
  $cheatButton2.text("Retornar")
  $cheatButton.css({
    cursor: 'pointer',
    color: 'white',
    'font-size': '14px',
...
async function cheat(seconds) {
  ...
  display_timespent_div.addEventListener('timer-finished', function () {
    ...
    addEndDialog("Intervenção física \n Levante-se e faça 10 polichinelos para manter seu corpo ativo.");
  })
...
(async function () {
  ...
  if (!is_new_session) {
    ...
    displayCountdown(seconds_selected, seconds_spent_at_most_recent_start)
  } else {
    main()
  }
})()
