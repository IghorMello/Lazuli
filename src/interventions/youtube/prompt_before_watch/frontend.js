require.ensure(['jquery', 'libs_frontend/tab_utils', 'libs_frontend/frontend_libs', 'libs_common/common_libs', 'libs_common/time_spent_utils', 'libs_frontend/intervention_log_utils', 'components/lazuli-logo-v2.deps', 'components/call-to-action-button.deps', 'components/positive-goal-site-button.deps', 'bower_components/paper-button/paper-button.deps'], async function (require) {
  ...
  function shadow_find(query) {
    var overlay_host = $('#lazuli_video_overlay')
  ...
  function set_video_duration() {
    ...
    if (!isNaN(duration)) {
      ...
      overlay.find('#message_text').html("Este vídeo possui " + minutes + " minutos e " + seconds + " segundos de duração. <br> Tem certeza de que deseja reproduzi-lo?");
      clearInterval(wait_for_video_duration);
      wait_for_video_duration = null
  ...
  function start_video_duration_setter() {
    ...
    if (!overlay.data('duration_set')) {
      overlay.find('#message_text').html('<span style="visibility: hidden">Este video possui XX minutos e XX segundos.<br>Tem certeza que quer rodar?</span>')
    }
  ...
  function divOverVideo(status) {
    ...
    $('#lazuli_video_overlay').remove()
    const $a = $('<div>')
    $(document.body).append($(wrap_in_shadow($a)).attr('id', 'lazuli_video_overlay'));
    ...
    $contentContainer.append('<lazuli-logo-v2>')
    ...
    if (status === 'begin') {
      ...
      get_seconds_spent_on_domain_today('www.youtube.com').then(function (secondsSpent) {
        ...
        shadow_find('#message_text').html("Você gastou " + mins + " minutos e " + secs + " segundos no Youtube hoje. <br> Tem certeza de que deseja continuar assistindo aos vídeos?");
      })
    }
  }
