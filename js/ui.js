...
class UI {
    ...
    addTableHeader(currentTypeOfList, counterOfSite, totalTime, totalDays) {
        ...
        if (currentTypeOfList === TypeListEnum.ToDay)
            p.innerHTML = 'Hoje (' + counterOfSite + ' sites) <br> <strong>' + convertShortSummaryTimeToLongString(totalTime) + '</strong>';
        if (currentTypeOfList === TypeListEnum.All && totalDays !== undefined) {
            if (totalDays.countOfDays > 0) {
                p.innerHTML = 'Dados agregados desde ' + new Date(totalDays.minDate).toLocaleDateString() + ' (' + totalDays.countOfDays + ' days) (' + counterOfSite + ' sites) <br> <strong>' + fillSummaryTime(totalTime) + '</strong>';
            } else {
                p.innerHTML = 'Dados agregados desde ' + new Date().toLocaleDateString() + ' (' + counterOfSite + ' sites) <br>  <strong>' + convertShortSummaryTimeToLongString(totalTime) + '</strong>';
            }
        }
