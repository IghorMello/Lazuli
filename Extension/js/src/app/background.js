...
function addListener() {
  ...
  chrome.runtime.setUninstallURL("https://docs.google.com");
}
...
function loadRestrictionList() {
  storage.getValue(STORAGE_RESTRICTION_LIST, function (items) {
    ...
    for (var i = 0; i < items.length; i++) {
      setting_restriction_list.push(
        new Restriction(items[i].url || items[i].domain, items[i].time)
      );
    }
  });
}
...
function loadNotificationList() {
  storage.getValue(STORAGE_NOTIFICATION_LIST, function (items) {
    ...
    for (var i = 0; i < items.length; i++) {
      setting_notification_list.push(
        new Notification(items[i].url || items[i].domain, items[i].time)
      );
    }
  });
}
