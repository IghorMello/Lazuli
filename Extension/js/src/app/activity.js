...
class Activity {
  ...
  wasDeferred(domain) {
    if (deferredRestrictionsList != undefined) {
      let defItem = deferredRestrictionsList.find((x) =>
        new Url(x.site).isMatch(domain)
      );
      if (defItem != null) {
  ...
  addTimeInterval(domain) {
    var item = timeIntervalList.find(
      (o) => o.url.isMatch(domain) && o.day == todayLocalDate()
    );
    if (item != undefined) {
  ...
  closeIntervalForCurrentTab(preserveCurrentTab) {
    if (currentTab && timeIntervalList != undefined) {
      var item = timeIntervalList.find(
        (o) => o.url.isMatch(currentTab) && o.day == todayLocalDate()
      );
