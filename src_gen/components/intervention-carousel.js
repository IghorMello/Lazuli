      ...
      create_intervention_dialog.addEventListener('upload_intervention', async function (event) {
        ...
        await chrome.identity.getProfileUserInfo(async function (author_info) {
          ...
          if (intervention_2_upload.displayname.trim() == "") {
            swal("Error: O nome da intervenção de upload não pode estar vazio!")
            return
          }
          ...
          try {
            ...
            if (response.success) {
              ...
              swal("Obrigado por compartilhar seu código! \NAqui está um link no qual você pode compartilhar seu código em particular: \n" + url)
              localStorage['uploaded_intervention_' + intervention_2_upload.name] = intervention_2_upload.code
              self.fire('intervention-added', {})
              //this.site_changed()
            } else {
              swal("Falha ao carregar alerta, por favor, abra um tíquete!")
            }
          }

          catch (err) {
            swal("Falha ao carregar alerta, por favor, abra um tíquete!", err)
          }
        });
      })
    }
