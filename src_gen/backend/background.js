...
(async function(){
  var dlog, ref$, addtokey_dictdict, getkey_dictdict, getkey_dict, setkey_dictdict, getDb, chrome_manifest, lazuli_version, developer_mode, unofficial_version, need_to_clear_cache, version_cached, localforage, store, storesystemjs, post_json, moment, get_goals, get_goal_info, get_enabled_goals, get_goal_target, get_goal_intervention_info, set_goals_enabled, set_default_goals_enabled, get_random_positive_goal, site_has_enabled_spend_less_time_goal, get_have_suggested_domain_as_goal, is_domain_unproductive, list_goals_for_location, list_nonpositive_goals_for_location, get_interventions, get_intervention_info, list_enabled_nonconflicting_interventions_for_location, list_all_enabled_interventions_for_location, list_available_interventions_for_location, get_intervention_parameters, is_it_outside_work_hours, set_default_generic_interventions_enabled, get_suggested_intervention_if_needed_for_url, enabledisable_interventions_based_on_difficulty, get_intensity_level_for_intervention, is_video_domain, choose_intervention_for_difficulty_level_and_goal, send_message_to_active_tab, send_message_to_tabid, get_active_tab_info, get_user_id, get_install_id, get_user_secret, list_currently_loaded_interventions_for_tabid, send_logging_enabled, get_basic_client_data, send_feature_disabled, setup_abtest_newuser, setup_abtest_olduser, make_tab_focused, focus_tab_by_pattern_if_available, num_times_notification_already_shown, num_times_to_pause_before_next_notification, show_finish_configuring_notification_if_needed, get_last_visit_to_website_timestamp, get_last_visit_to_chrome_store_timestamp, get_all_message_handlers, localget, remoteget, systemjsget, start_syncing_all_data, make_wait_token, wait_for_token, finished_waiting, get_days_since_epoch, get_new_session_id_for_domain, url_to_domain, add_tab_navigation_event, run_every_timeperiod, as_array, as_dictset, localstorage_getjson, localstorage_setjson, localstorage_getbool, baseline_time_per_session_for_domain, promiseDebounce, gexport, gexport_module, set_active_interventions_for_domain_and_session, get_progress_on_enabled_goals_today, get_progress_on_goal_this_week, is_lazuli_enabled_sync, ensure_history_utils_data_cached, log_impression_internal, log_goal_suggestion, add_log_history, log_feedback_internal, sleep, printable_time_spent_short, one_random_intervention_per_enabled_goal, insert_css, running_background_scripts, load_background_script, cached_systemjs_code, cached_bundle_code, execute_content_scripts_for_intervention, find_webpack_chunks, load_intervention_for_session_id, load_intervention_list, list_loaded_interventions, get_session_id_for_tab_id_and_domain, tab_id_to_loaded_interventions, domain_to_prev_enabled_interventions, page_was_just_refreshed, load_intervention_for_location, getLocation, split_list_by_length, iframed_domain_to_track, tabs_to_listen_for_focus, css_packages, css_files_cached, message_handlers, prev_domain, domain_changed, tab_id_to_domain_to_session_id, tab_id_to_url, list_domain_to_session_ids, get_last_duolingo_progress_update_time, update_duolingo_progress, navigation_occurred, reward_display_base_code_cached, message_handlers_requiring_tab, current_idlestate, ref1$, prev_browser_focused, updates_to_sync, mobile_server, goal_suggestion_threshold, suggest_goal_base_code, domain_to_time_last_suggested_goal, get_lazuli_uninstall_url, decode_lazuli_uninstall_url_data, set_lazuli_uninstall_url, open_lazuli_uninstall_url, num_times_restart_failed_due_to_loaded_interventions, num_times_restart_failed_due_to_loaded_interventions_active, num_times_restart_failed_due_to_lazuli_tab_open, num_times_restart_failed_due_to_lazuli_tab_open_active, restart_failed_priority_to_counts, record_restart_failed, try_to_restart_lazuli_now, lazuli_restarter_running, start_trying_to_restart_lazuli, semver, check_if_update_available_and_run_update, update_available_version, url_to_open_on_next_start;
  ...
  ref$ = require('libs_backend/db_utils'), addtokey_dictdict = ref$.addtokey_dictdict, getkey_dictdict = ref$.getkey_dictdict, getkey_dict = ref$.getkey_dict, setkey_dictdict = ref$.setkey_dictdict, getDb = ref$.getDb;
  (await getDb());
  ...
  if (!developer_mode || localStorage.getItem('devmode_use_cache') === 'true') {
    ...
    if (need_to_clear_cache || !developer_mode) {
      if (!need_to_clear_cache) {
        if (!developer_mode) {
          version_cached = localStorage.getItem('lazuli_version');
          if (version_cached !== lazuli_version) {
      ...
      if (need_to_clear_cache) {
        ...
        localStorage.setItem('lazuli_version', lazuli_version);
      }
    }
  }
  ...
  ref$ = require('libs_backend/background_common'), send_message_to_active_tab = ref$.send_message_to_active_tab, send_message_to_tabid = ref$.send_message_to_tabid, get_active_tab_info = ref$.get_active_tab_info, get_user_id = ref$.get_user_id, get_install_id = ref$.get_install_id, get_user_secret = ref$.get_user_secret, list_currently_loaded_interventions_for_tabid = ref$.list_currently_loaded_interventions_for_tabid;
  ref$ = require('libs_backend/logging_enabled_utils'), send_logging_enabled = ref$.send_logging_enabled, get_basic_client_data = ref$.get_basic_client_data, send_feature_disabled = ref$.send_feature_disabled;
  ref$ = require('libs_backend/abtest_utils'), setup_abtest_newuser = ref$.setup_abtest_newuser, setup_abtest_olduser = ref$.setup_abtest_olduser;
  ...
  out$.show_finish_configuring_notification_if_needed = show_finish_configuring_notification_if_needed = async function(){
    ...
    notification = new Notification('Finish setting up Lazuli', {
      icon: chrome.extension.getURL('icons/icon_128.png'),
      body: 'Click here to finish setting up Lazuli',
  ...
  out$.get_last_visit_to_website_timestamp = get_last_visit_to_website_timestamp = async function(){
    ...
    for (i$ = 0, len$ = history_search_results.length; i$ < len$; ++i$) {
      ...
      if (search_result.url.startsWith('https://lazuli.stanford.edu')) {
  ...
  out$.get_last_visit_to_chrome_store_timestamp = get_last_visit_to_chrome_store_timestamp = async function(){
    ...
    history_search_results = (await new Promise(function(it){
      return chrome.history.search({
        text: 'https://chrome.google.com/webstore/detail/lazuli/obghclocpdgcekcognpkblghkedcpdgd',
        startTime: 0
      }, it);
    }));
    last_visit_timestamp = -1;
    for (i$ = 0, len$ = history_search_results.length; i$ < len$; ++i$) {
      search_result = history_search_results[i$];
      if (search_result.url.startsWith('https://chrome.google.com/webstore/detail/lazuli/obghclocpdgcekcognpkblghkedcpdgd')) {
        if (search_result.lastVisitTime > last_visit_timestamp) {
          last_visit_timestamp = search_result.lastVisitTime;
        }
      }
  ...
  (async function(){
    var user_id, tab_info, last_visit_to_website_timestamp, last_visit_to_chrome_store_timestamp, need_to_create_new_tab, install_source, install_data;
    ...
    if (tab_info != null) {
      if (tab_info.url === 'https://lazuli.netlify.com/#installing' || tab_info.url === 'https://lazuli.stanford.edu/#installing' || tab_info.url === 'https://lazuli.github.io/#installing' || tab_info.url === 'http://lazuli.netlify.com/#installing' || tab_info.url === 'http://lazuli.stanford.edu/#installing' || tab_info.url === 'http://lazuli.github.io/#installing') {
    ...
    post_json('https://lazuli.herokuapp.com/add_install', install_data);
  })();
  get_all_message_handlers = require('libs_backend/expose_lib').get_all_message_handlers;
  require('libs_backend/expose_backend_libs');
  ref$ = require('libs_common/cacheget_utils'), localget = ref$.localget, remoteget = ref$.remoteget, systemjsget = ref$.systemjsget;
  start_syncing_all_data = require('libs_backend/log_sync_utils').start_syncing_all_data;
  ref$ = require('libs_common/wait_utils'), make_wait_token = ref$.make_wait_token, wait_for_token = ref$.wait_for_token, finished_waiting = ref$.finished_waiting;
  get_days_since_epoch = require('libs_common/time_utils').get_days_since_epoch;
  get_new_session_id_for_domain = require('libs_common/time_spent_utils').get_new_session_id_for_domain;
  url_to_domain = require('libs_common/domain_utils').url_to_domain;
  add_tab_navigation_event = require('libs_backend/session_utils').add_tab_navigation_event;
  run_every_timeperiod = require('libs_common/common_libs').run_every_timeperiod;
  ref$ = require('libs_common/collection_utils'), as_array = ref$.as_array, as_dictset = ref$.as_dictset;
  ref$ = require('libs_common/localstorage_utils'), localstorage_getjson = ref$.localstorage_getjson, localstorage_setjson = ref$.localstorage_setjson, localstorage_getbool = ref$.localstorage_getbool;
  baseline_time_per_session_for_domain = require('libs_common/gamification_utils').baseline_time_per_session_for_domain;
  moment = require('moment');
  promiseDebounce = require('promise-debounce');
  ref$ = require('libs_common/gexport'), gexport = ref$.gexport, gexport_module = ref$.gexport_module;
  set_active_interventions_for_domain_and_session = require('libs_backend/intervention_manager').set_active_interventions_for_domain_and_session;
  ref$ = require('libs_backend/goal_progress'), get_progress_on_enabled_goals_today = ref$.get_progress_on_enabled_goals_today, get_progress_on_goal_this_week = ref$.get_progress_on_goal_this_week;
  is_lazuli_enabled_sync = require('libs_backend/disable_lazuli_utils').is_lazuli_enabled_sync;
  ensure_history_utils_data_cached = require('libs_common/history_utils').ensure_history_utils_data_cached;
  ref$ = require('libs_backend/log_utils'), log_impression_internal = ref$.log_impression_internal, log_goal_suggestion = ref$.log_goal_suggestion, add_log_history = ref$.add_log_history, log_feedback_internal = ref$.log_feedback_internal;
  sleep = require('libs_common/common_libs').sleep;
  printable_time_spent_short = require('libs_common/time_utils').printable_time_spent_short;
  one_random_intervention_per_enabled_goal = require('libs_backend/intervention_selection_algorithms').one_random_intervention_per_enabled_goal;
  /*
  ...
  */
  ...
  find_webpack_chunks = function(code){
    ...
    chunks = [];
    index = code.indexOf('async function lazuli_intervention_main_function(){');
    if (index === -1) {
      return [];
    }
    index += 'async function lazuli_intervention_main_function(){'.length;
  ...
  */
  ...
  ref$ = require('libs_backend/duolingo_utils'), get_last_duolingo_progress_update_time = ref$.get_last_duolingo_progress_update_time, update_duolingo_progress = ref$.update_duolingo_progress;
    ...
    if (!is_from_history && changeInfo != null && changeInfo.status === 'loading') {
      return load_intervention_for_location(url, tabId).then(function(){
        ...
        if (loaded_interventions != null && loaded_interventions.length > 0) {
          ...
          if (is_lazuli_enabled_sync() && !is_it_outside_work_hours()) {
            return chrome.browserAction.setIcon({
              tabId: tabId,
              path: chrome.extension.getURL('icons/icon.svg')
            });
  ...
  */
  ...
  setInterval(async function(){
    var user_id, timestamp, domain_to_num_seconds_to_increment, i$, ref$, len$, ref1$, domain, seconds;
    ...
    for (i$ = 0, len$ = (ref$ = updates_to_sync).length; i$ < len$; ++i$) {
      ref1$ = ref$[i$], domain = ref1$.domain, seconds = ref1$.seconds;
      if (domain_to_num_seconds_to_increment[domain] == null) {
        domain_to_num_seconds_to_increment[domain] = 0;
      }
      domain_to_num_seconds_to_increment[domain] += seconds;
    }
    updates_to_sync.length = 0;
    return post_json(mobile_server + '/addsessiontototal', {
      userid: user_id,
      timestamp: timestamp,
  ...
  setInterval (->>
    ...
    if not active_tab?
      return
    if active_tab.url.startsWith('chrome://') or active_tab.url.startsWith('chrome-extension://') # ignore Tempo gasto no extension pages
      return
    current_domain = url_to_domain(active_tab.url)
  ...
  */
  ...
  require('libs_backend/notification_timer');
  (await get_goal_intervention_info());
  (await get_goals());
  (await get_enabled_goals());
  get_lazuli_uninstall_url = async function(){
    ...
    uninstall_url_base = 'https://lazuli.github.io/bye#';
    uninstall_url = uninstall_url_base;
    uninstall_url_data = {};
    uninstall_url_data.v = lazuli_version;
    if (chrome.runtime.id === 'obghclocpdgcekcognpkblghkedcpdgd') {
      ...
      uninstall_url_base = 'https://lazuli.netlify.com/bye#';
  ...
  decode_lazuli_uninstall_url_data = async function(url){
  ...
  set_lazuli_uninstall_url = async function(){
    var lazuli_uninstall_url;
    lazuli_uninstall_url = (await get_lazuli_uninstall_url());
    return chrome.runtime.setUninstallURL(lazuli_uninstall_url);
  };
  out$.open_lazuli_uninstall_url = open_lazuli_uninstall_url = async function(){
    var uninstall_url;
    uninstall_url = (await get_lazuli_uninstall_url());
    return chrome.tabs.create({
      url: uninstall_url
    });
  };
  if (!developer_mode) {
    (await set_lazuli_uninstall_url());
  }
  num_times_restart_failed_due_to_loaded_interventions = 0;
  num_times_restart_failed_due_to_loaded_interventions_active = 0;
  num_times_restart_failed_due_to_lazuli_tab_open = 0;
  num_times_restart_failed_due_to_lazuli_tab_open_active = 0;
  ...
  out$.try_to_restart_lazuli_now = try_to_restart_lazuli_now = async function(){
  ...
  lazuli_restarter_running = false;
  out$.start_trying_to_restart_lazuli = start_trying_to_restart_lazuli = async function(){
    if (lazuli_restarter_running) {
      return;
    }
    lazuli_restarter_running = true;
    for (;;) {
      (await try_to_restart_lazuli_now());
      (await sleep(60000));
    }
  };
  semver = require('semver');
  check_if_update_available_and_run_update = require('libs_backend/lazuli_update_utils').check_if_update_available_and_run_update;
  ...
  if (update_available_version != null) {
    ...
    if (semver.gte(lazuli_version, update_available_version)) {
      localStorage.removeItem('extension_update_available_version');
    }
  }
  chrome.runtime.onUpdateAvailable.addListener(function(update_details){
    if (semver.valid(update_details.version)) {
      ...
      return start_trying_to_restart_lazuli();
    }
  });
  ...
  url_to_open_on_next_start = localStorage.getItem('lazuli_open_url_on_next_start');
  if (url_to_open_on_next_start != null) {
    localStorage.removeItem('lazuli_open_url_on_next_start');
    chrome.tabs.create({
