...
const remove_feed_once_available = run_only_one_at_a_time((callback) => {
   ...
   */
  $('<style class="lazuli_youtube_feeddiet_style">')
    .prop('type', 'text/css')
    .html('\
...
function show_buttons(target_selector) {
  ...
  if ($(target_selector).find('.lazuli_inserted_div').length > 0) {
    return
  }

  let lazuli_div = $('<div style="width: 100%; text-align: center">')

  lazuli_div.append($('<lazuli-logo-v2>'))
  lazuli_div.append($('<br>'))

  let showFeedButton = $('<paper-button style="background-color: #415D67; color: white; -webkit-font-smoothing: antialiased; font-size: 8px; margin-top: 10px; box-shadow: 2px 2px 2px #888888;">Mostrar feed desta vez</paper-button>')

  showFeedButton.on('click', disable_intervention)
  lazuli_div.append(showFeedButton)
  let lazuli_div_wrapper = $(wrap_in_shadow(lazuli_div))
    .addClass('lazuli_inserted_div').css('margin-top', '100px')
  $(target_selector).prepend(lazuli_div_wrapper)
}

function disable_intervention() {
  // Remove the previously inserted buttons and CSS
  $('.lazuli_inserted_div').remove()
  $('.lazuli_youtube_feeddiet_style').remove();
